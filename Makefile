...
.PHONY: build build-windows build-wsl-launcher build-linux deploy-wsl clean run

build: build-windows build-wsl-launcher build-linux deploy-wsl
...
clean:
	rm -f $(BINARY) $(WSL_BINARY) $(LINUX_BINARY) cmd/any-ai-cli/rsrc_windows_*.syso cmd/any-ai-cli-wsl/rsrc_windows_*.syso
