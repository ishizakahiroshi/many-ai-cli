...
> 最終更新: 2026-08-17(月) — 複数サブスクリプション管理の設計原則を 1 節追加（設定ディレクトリを env で切る・token を持たない）。同節の usage remaining を「問い合わせて取る」と「押し出されてくる」で書き分け。観測コード節を新 schema（日付 due・`sharedFiles`）と 4 件目の実例へ更新
