...
> 最終更新: 2026-08-19(水) — 見送り台帳の件数を 6 件へ更新（D-06 OpenRouter を route として載せる案を追加）。2026-08-17: 複数サブスクリプション管理の設計原則を 1 節追加（設定ディレクトリを env で切る・token を持たない）。同節の usage remaining を「問い合わせて取る」と「押し出されてくる」で書き分け。観測コード節を新 schema（日付 due・`sharedFiles`）と 4 件目の実例へ更新
...
**見送った方針は `docs/local/reference_declined-directions.md` に台帳としてまとめてある。** Gemini の wrap・新規 provider・セッションカンバン・本節・Copilot/Cursor の複数サブスクリプション対応・OpenRouter を route として載せる案の 6 件が入っており、各項目に「再検討してよい条件」と「再検討の根拠にしてはいけないもの」を書いてある。**同種の提案を出す前に必ずここを読む。** 見送りは永久否定ではないので、条件が満たされているなら再検討してよい。
...
- `due` を必ず書く。**形式は日付（`YYYY-MM-DD`・UTC の暦日で比較）**（2026-08-17 に版数指定から移行）。過ぎたらブロックされる。延ばすなら `due` を更新して `reason` に「なぜまだ要るか」を書き足す（黙って延ばさない）
- **ファイルは `files` と `sharedFiles` に分けて書く。** `files` はその観測コード専用のファイル（撤去すればファイルごと消える）で、登録検査と撤去検査の**両方**で見られる。`sharedFiles` は観測コードが間借りしている共有ファイル（`server.go` 等）で、登録検査だけで見る。**共有ファイルを `files` に載せると `removed` にした瞬間に「まだ追跡されている」で永久に落ちる**（ファイル自体は消せないため）
- 撤去したら `status: removed` にする。実体が残っていれば検査で落ちる

**「原因が確定したら消す」とコメントに書くだけでは消えない。** v0.5.1 の `/api/debug/batch-log`、v0.5.x の入力トレース（監査 A-01）、v0.6.0 の `/api/debug/ui-log` と `ui_input_trace`、そして 2026-08-17 に見つかった `input_trace` と、**4 回続けて出荷直前まで残った**。4 件中 3 件（`ui_input_trace` / `input_trace` / v0.5.x の入力トレース）が**専用ファイルを持たず共有ファイルに分散していた**ため、同種の撤去作業から取り残されている。`sharedFiles` はこの再発を止めるために足した。

**検査が green でも「守備範囲の外」がある。** `scripts/check-instrumentation.mjs` が未登録の観測コードを見つける手段は 3 つだけ。ファイル名（`debug_*.go` / `*_trace.go` / `debug-*.ts`）、本文に含まれる語（「一時観測」「観測専用」「観測用エンドポイント」「原因が確定したら」「原因確定後に撤去」）、`/api/debug/` のエンドポイント。**`input_trace` は「観測専用の一時コード」とコミットメッセージにだけ書かれ、ソース側の文言が「入力経路の診断」だったので素通りした**（ファイル名も `input_gate.go`）。**観測コードにはソースのコメント側にも上記の語を書く。**
