...
セッションログと SQLite ベースの Workbench 履歴はローカルの private storage（可能な箇所は directory `0700` / file `0600`）に保存されますが、プロンプト、ファイルパス、ユーザー入力テキストを含み得ます。既知の token 形式は PTY 出力・ユーザー入力履歴の保存前に redaction し、Workbench export も既定で redaction しますが、ヒューリスティックです。誤って機密情報を貼った場合は、設定から保存済み履歴を削除するか `~/.any-ai-cli/logs/` を削除してください。
