...
複数のターミナルで AI コーディング CLI を並列実行していると、どのセッションが止まっているか気づけず、ターミナルを何度も見にいくことになります。`many-ai-cli` は各 CLI を PTY でラップし、承認待ち・タスク完了・エラーを検出した瞬間にデスクトップ／スマホへ通知します。さらにブラウザの Hub UI で承認操作・進捗監視を一元的に行えます。CLI 本体の機能はそのままで、通知と承認 GUI を追加する設計です。

**承認の自動化が進んでも、この役割は残ります。** Claude Code の auto mode のように危険な操作だけを止める方式が広がると、承認プロンプトの頻度は下がります。ただし止まらなくなるわけではなく、セッションが長く黙って走ったあとに突然 1 回だけ止まる形に変わります。並列運用では、この「たまに止まる」ほうが気づきにくくなります。加えて承認モデルは CLI ごとに異なり、同時に自動化されるわけでもないため、複数ベンダーを並べて使うなら状態を 1 か所へ集める層が必要になります。`many-ai-cli` は承認ボタンを代行する道具ではなく、**止まっている事実を検知して届ける**道具として設計しています。
...
Claude Code と Codex の子では、spawn 確認ダイアログに **「このフォルダを 〈CLI 名〉 の信頼済みとして登録する」**（既定でチェックあり）が出ます。チェックを入れたまま承認すると、Hub は子を起動する直前に、その CLI が信頼の確認に Yes と答えたときに書くのと同じファイル・同じ形で、子のフォルダを信頼済みとして記録します。Claude Code は `.claude.json` の `projects` に `hasTrustDialogAccepted: true` の項目を、Codex は `config.toml` に `trust_level = "trusted"` の `[projects.'<フォルダ>']` 表を書きます（subscription profile で動く子は profile 側の設定へ書きます）。これで子は確認を出さずに始まります。書くのは項目がまだ無いときだけで、既にある項目（信頼しないと答えたフォルダを含む）は書き換えません。これを指定できるのはダイアログでのあなたの承認だけで、AI の spawn 要求からは指定できません。書き込みに失敗しても子は起動し、子のセッションで確認が出ます。チェックを外したときと、ダイアログが出ない起動では、子は信頼の確認で待ちます。待っていることは board（指揮者がいれば指揮者にも）へ 1 回だけ知らせ、子のセッションで答えればすぐ指示が始まります。
...
> **注意（ベータ／ドラフト版）** — スマホ向け UI は v0.3.x で試験提供中です。レイアウト・操作感・通知の挙動は今後のバージョンで変更される可能性があります。フィードバックは GitHub Issue でお願いします。
...
- **Whisper（ローカル）** — 音声データを外部サービスへ送りたくない人向け。録音は Hub 経由でローカルの whisper.cpp サーバに渡され、外には出ません（`voice.whisper.server_url` を `127.0.0.1` / `localhost` のままにしている限り）。精度はモデルと CPU 性能次第。
- **ブラウザ内蔵** — Chrome / Edge の Web Speech API。**音声データは Google / Microsoft の認識サーバへ送信されます**。精度・反応速度は良好。プライバシーより精度・手軽さを優先する場面向け。
- **スマホからの利用** — Hub の音声入力エンジンを使わず、**スマホ OS の音声入力 / IME**（iPhone の場合は `Gboard` 等のサードパーティ IME も含む）を使う選択肢もあります。クラウド処理である点はブラウザ内蔵と同じですが、スマホは IME 経由のほうが反応が良いことが多いです。

要するに「音声を外に出したくない → Whisper」「精度・手軽さ優先 → ブラウザ内蔵 or スマホ IME」で選び分けてください。
