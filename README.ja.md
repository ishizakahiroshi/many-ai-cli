...
- **承認パネル統合**: Claude Code / Codex CLI の承認待ちをブラウザ上のアクションバーで処理
- **複数質問の一括承認**: 1つの承認ブロック内の番号付き質問を選択し、まとめて PTY へ送信
- **リアルタイム PTY 表示**: xterm.js + WebSocket で CLI 出力を表示
- **Files タブ**: プロジェクトファイルをツリー表示し、Markdown / コードのプレビュー、パスコピー、リネーム、移動を実行
- **Git ビュー**: ブランチ履歴、commit 詳細、変更ファイル、diff を checkout なしで確認
...
- 実機で動作検証済み: Windows
- WSL 連携は Windows 用ランチャー `any-ai-cli-wsl.exe` で対応
- 実機で十分に未検証: ネイティブ Linux / ネイティブ macOS

Linux / macOS でも動作する想定ですが、v0.2.0 時点では実機での十分な検証は未実施です。
...
1. リリースページから `any-ai-cli.exe`（macOS / Linux はバイナリ）をダウンロードする
...
`ANY_AI_CLI_AUTO=1` が設定されていないシェルでは、`claude` / `codex` はそのまま元のコマンドとして動作します。グローバルな `.bashrc` 等は改変しません。
...
- **タブ**: ターミナルセッション、Files タブ、Git タブを同じメイン領域で切り替え。Hub 再起動後の復元にも対応
...
設定パネル → **音声入力** でウェイクワードモードを ON にすると、指定フレーズを発話したときだけ音声入力を開始できます。全セッション向けの待機は `Alt+W`、セッション単位の待機は入力欄横の 🎧 ボタンから切り替えます。

同じ設定セクションで「終了検知の待ち時間」も変更できます。Chrome の音声認識が無音で自動終了した後でも、直近の発話から指定秒数以内なら認識を再開します。
...
| `Alt+W` | 全セッション向けウェイクワード待機のON/OFFを切り替え |
...
`user_prefs:` には音声入力、ウェイクワード、通知音、承認時の自動切替、クイックコマンド、利用リンク、お気に入り、セッション順、spawn 既定値などのユーザー機能設定が保存されます。ブラウザの localStorage だけに依存しないため、ポート変更や WSL ランチャー経由でも設定が維持されます。

承認検出パターンは provider ごとに `official` / `custom` プロファイルを持ちます。`official` は GitHub 上の `resources/approval-patterns/{claude,codex,common}.md` から起動時に取得・キャッシュされ、`custom` はユーザー編集用です。
...
- **スラッシュコマンド一覧の取得（Hub 本体の通信）**: スラッシュコマンドピッカーを開くと、Hub は `https://raw.githubusercontent.com/ishizakahiroshi/any-ai-cli/main/resources/slash-commands/{claude,codex}.md` を取得し、24 時間キャッシュします。取得元 URL は設定パネルの **スラッシュコマンドソース** から変更可能で、ローカルファイルパスを指定することもできます。
- **承認検出パターンの取得（Hub 本体の通信）**: Hub 起動時に、公式の承認検出パターンを `https://raw.githubusercontent.com/ishizakahiroshi/any-ai-cli/main/resources/approval-patterns/{claude,codex,common}.md` から取得し、24 時間キャッシュする場合があります。取得元 URL は config で上書きできます。
- **wrap 対象 CLI の API 通信（CLI 自身の通信）**: ラップ対象である Claude Code / Codex CLI 自身は、それぞれのベンダー API（Anthropic / OpenAI）と HTTPS で直接通信します。`any-ai-cli` は PTY の入出力をローカル WebSocket で中継するだけで、これらの API 通信を傍受・記録・プロキシすることはありません。元の CLI のネットワーク挙動がそのまま適用されます。
...
`any-ai-cli` は **同一マシン上での利用** を前提に設計されています。以下は絶対に行わないでください。
