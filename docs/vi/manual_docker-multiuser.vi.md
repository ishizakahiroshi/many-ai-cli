...
Hướng dẫn vận hành nhiều người trên remote server XServer (tên server `admin`) theo mô hình «1 user = 1 Docker container» cho `many-ai-cli`. Đường dẫn vật lý phía server (`/opt/any-ai-cli/` `/srv/any-ai-cli/`), tên compose project và docker volume **vẫn dùng tên cũ `any-ai-cli`** trong vận hành thực tế, nên tài liệu này cũng ghi theo thực tế đó. Lịch sử thiết kế / quyết định: `docs/local/archive/v0.3.2/plan_docker-multiuser-isolation.md`.
