...
> 最終更新: 2026-05-31(日) 09:11:38 — Copilot CLI の slash command source を追加

Claude Code / Codex CLI / GitHub Copilot CLI に新しいスラッシュコマンドが追加された際、ダッシュボードのスラッシュコマンドピッカーに反映させるための運用メモ。

## 仕組み（前提）

- 一覧の正本は `resources/slash-commands/claude.md` / `resources/slash-commands/codex.md` / `resources/slash-commands/copilot.md`（markdown テーブル）
...
```powershell
# develop でコミット
git add resources/slash-commands/claude.md resources/slash-commands/codex.md resources/slash-commands/copilot.md
...
https://raw.githubusercontent.com/ishizakahiroshi/any-ai-cli/main/resources/slash-commands/copilot.md
```
...
- ピッカーは **アクティブなセッションの provider**（claude / codex / copilot）に応じて対象が切り替わる。それぞれの provider のセッションでピッカーを開いて `⟳` する。
