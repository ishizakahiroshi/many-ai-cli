...
`plan_spawn-model-picker-ollama.md` の実装により、any-ai-cli v0.1.4+ では **Hub 画面の spawn フォームから Ollama Cloud / Local モデルを直接選択して起動できる**。env や profile を親 shell で設定する必要はない（spawn 時に Hub が必要な env を自動注入する）。
...
- バックエンド `/api/models` が以下を集約して返す:
  ...
  - Ollama Local: `http://localhost:11434/api/tags`（60s キャッシュ） + `~/.any-ai-cli/config.yaml` の `local_models:` 手書きを merge
...
1. `any-ai-cli serve` で Hub を起動（ブラウザ自動起動）
...
- daemon が落ちていても `~/.any-ai-cli/config.yaml` に手書きで以下を追記すれば一覧に出る:
...
将来、provider 汎化が完了した段階で Hub UI / ログに以下のフィールドを表示することを想定する（v0.1.3 時点では未実装、設計メモとして記録）。
...
| Hub 経由だと env が効かない | 既存 Hub プロセスの env が古い | `any-ai-cli stop && any-ai-cli serve` |
...
any-ai-cli v0.1.4+ では Hub 画面方式が主だが、以下の場合は親 shell で env を設定する旧方式が使える。

- `any-ai-cli` を経由せず直接 `claude` / `codex` を起動するケース
...
### any-ai-cli serve を shell env 込みで起動

shell 方式で any-ai-cli 全体を Ollama 化するには、env を export 済みの shell から Hub を起動する:
