...
開発端末は Windows 11。`many-ai-cli` 自体はクロスプラットフォーム（Win/Mac/Linux）だが、本ドキュメントはユーザー環境固有の手順をまとめる。
...
# 3. 起動確認
./many-ai-cli.exe serve

# 4. 別ターミナルでラッパー起動
./many-ai-cli.exe wrap claude
```
...
- `os.UserHomeDir()` は Windows で `C:\Users\<name>` を返す。`~/.many-ai-cli/` 表記は Windows でも `C:\Users\<name>\.many-ai-cli\` の意味で扱う
