...
| `/auto-run` | Toggle Run Everything or show current status. | When automating execution without manual approval. |
| `/bedrock` | Configure Bedrock when the Bedrock feature is enabled. | When setting up AWS Bedrock integration. |
| `/clear` | Start a new chat session. | When beginning a fresh conversation. |
| `/compress` | Summarize the conversation to reduce context usage. | When managing token limits in long discussions. |
...
| `/cursor` | Open the repository Git root in Cursor. | When you want the project in the editor. |
...
| `/new` | Start a fresh chat session. | When beginning a conversation without history. |
| `/new-chat` | Start a fresh chat session. | When beginning a conversation without history. |
| `/newchat` | Start a fresh chat session. | When beginning a conversation without history. |
| `/open` | Open the repository's Git root in Cursor. | When jumping into the project editor. |
...
| `/run` | Enter Shell Mode for terminal commands. | When executing terminal operations. |
| `/run-everything` | Toggle automatic execution or check its current status. | When controlling whether commands run without confirmation. |
...
| `/sh` | Enter Shell Mode for terminal commands. | When executing terminal operations. |
| `/shell` | Enter Shell Mode. | When running terminal commands directly. |
...
| `/summarize` | Summarize the conversation to reduce context. | When token usage is becoming excessive. |
